...
const ResourcePlayer = ({ resources, activeIdx, setActiveIdx, isMobile, onOpenModal, inline, hideExternalLink }) => {
  ...
  // Handle cleanup on UNMOUNT only to keep player alive during resource switching
  useEffect(() => {
    return () => {
  ...
  const initPlayer = () => {
    ...
    window.YT_INSTANCES[playerElementId] = new window.YT.Player(playerElementId, {
      videoId: ytId,
      playerVars: {
        autoplay: 1, // Autoplay when switching
        controls: 0,
        disablekb: 1,
        fs: 0,
        iv_load_policy: 3,
      ...
      events: {
        onReady: (event) => {
          ...
          setPlayer(p);
          // Safety: check if ytId changed during init
          if (ytIdRef.current && ytIdRef.current !== ytId) {
             p.loadVideoById(ytIdRef.current);
          }
          ...
          if (ytIdRef.current) {
            const saved = localStorage.getItem(`yt_pos_${ytIdRef.current}`);
            if (saved) p.seekTo(parseFloat(saved), true);
          }
        },
        onStateChange: (event) => {
  ...
  const topMaskHeight = isMobile ? '70px' : '75px';
  ...
  return (
    ...
    >
      <div style={{ flex: 1, position: 'relative', background: themeColor, overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        {/* YOUTUBE LAYER - Always present to avoid removeChild errors and allow reuse */}
        <div style={{ 
          width: '100%', height: '100%', position: 'absolute', inset: 0, 
          display: ytId ? 'flex' : 'none', 
          alignItems: 'center', justifyContent: 'center',
          zIndex: ytId ? 1 : -1 
        }}>
          <div style={{ width: '100%', aspectRatio: '16/9', position: 'relative', overflow: 'hidden', background: themeColor }}>
            <div id={`yt-player-${inline ? 'inline' : 'modal'}`} style={{ width: '100.2%', height: '100.2%', position: 'absolute', top: '-0.1%', left: '-0.1%' }}></div>
            {playerState !== 1 && (
              <div
                onClick={togglePlay}
        ...
        {/* NON-YOUTUBE LAYER - Persistent to avoid DOM reconciliation issues */}
        <div style={{ 
          width: '100%', height: '100%', position: 'absolute', inset: 0, 
          display: !ytId ? 'flex' : 'none',
          alignItems: 'center', justifyContent: 'center',
          ...
          {driveUrl ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '20px', padding: '40px', textAlign: 'center' }}>
              <FileText size={64} style={{ opacity: 0.2 }} />
              <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                <p style={{ margin: 0, fontSize: '1.1rem', fontWeight: 'bold' }}>{res.title || 'Внешний ресурс'}</p>
                <p style={{ margin: 0, opacity: 0.6, fontSize: '0.9rem' }}>Этот материал нельзя отобразить внутри приложения</p>
              </div>
              <button
                onClick={openExternal}
                style={{
                  display: 'flex', alignItems: 'center', gap: '10px',
                  padding: '12px 25px', background: 'var(--primary-color)', color: 'white',
                  borderRadius: '12px', border: 'none', cursor: 'pointer', fontWeight: 'bold'
                }}
              >
                <ExternalLink size={20} /> Открыть оригинал
              </button>
            </div>
          )}
        </div>
