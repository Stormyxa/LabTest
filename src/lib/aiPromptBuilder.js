 ...
 * @returns {Promise<string>} - Full prompt text ready to copy
 */
export const buildStudentPrompt = async (userId, viewerRole = 'student', viewerProfile = null) => {
  ...
  return await fetchWithCache(cacheKey, async () => {
    ...
    if (!attempts || attempts.length === 0) {
      return buildEmptyPrompt(displayName, `${cityName}, ${schoolName}, ${className}`, viewerRole);
    }
...
На основе JSON-данных ниже выполни:
 ...
 * @returns {string} - Full prompt text ready to copy
 */
export const buildQuizPromptFromData = ({ quiz, filteredResults, scopeLabel, cities, schools, classes }) => {
  if (!quiz || !filteredResults || filteredResults.length === 0) {
    return 'Нет данных для анализа. Убедитесь, что выбран тест и есть результаты по текущим фильтрам.';
  }
...
**Цель**: Провести детальный анализ результатов одного теста по группе учеников. Помочь учителю понять, какие вопросы/темы вызывают затруднения и как скорректировать обучение.
...
export const buildClassPrompt = async (classId) => {
  ...
  return await fetchWithCache(cacheKey, async () => {
    ...
    if (!results || results.length === 0) return `Класс ${cls.name} — нет результатов за последние ${DATA_PERIOD_DAYS} дней.`;
